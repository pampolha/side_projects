...
const distubeEvents = distube =>
{
    distube.on('searchResult', (message, result) => 
    {
        ...
        {
            embed.addField(`${index + 1}:`, `"**${song.name.replaceAll(/\|\||~~|[*`]|^> /gim, '\u200B')}**" - \`${song.formattedDuration}\``);
        });
    ...
    {
        return message.channel.send(`"**${song.name.replaceAll(/\|\||~~|[*`]|^> /gim, '\u200B')}**" - \`${song.formattedDuration}\` foi adicionado à queue por: ${song.user}`);
    });
    ...
    distube.on('playSong', (message, queue, song) =>
    {
        ...
        .addField(`"**${song.name.replaceAll(/\|\||~~|[*`]|^> /gim, '\u200B')}**" - \`${song.formattedDuration}\``, `pedido por ${song.user}`)
        .setURL(song.url);
